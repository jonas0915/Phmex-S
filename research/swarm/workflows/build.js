...
// order, never merges or pushes, and never edits the frozen spec or signal.py. It ends
// after the reviewer seat (plus a kb reconcile row) with the branch name and the list
// of files changed. It leaves the working tree checked out on the build branch.
//
...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Review', detail: 'independent reviewer (judge model) + full suite; one fix round at most' },
    { title: 'Reconcile', detail: 'kb row (SURVIVORS / DEAD_LIST / LESSONS) + kb_check; stop before Gate B' },
  ],
}
...
const NOW = A.now || (A.today ? `${A.today}T00:00:00Z` : null)          // ISO UTC; registered_ts source
...
// JUDGE_MODEL is applied to exactly one seat: the reviewer (both rounds). Never the
// prereg writer, the holdout reader/recorder, the implementer, the fixer or the reconciler.
...
- Bot trading code (bot.py, strategies.py, risk_manager.py, exchange.py, config.py, strategy_slot.py, any *_slot.py) is modified ONLY on git branch ${BRANCH}, ONLY in these files: ${ALLOWED_FILES.join(', ')}, ONLY by the IMPLEMENT and FIX seats. Every other seat is read-only on code.
...
- PASS is never a promotion: it makes the thesis PASS-ELIGIBLE for an owner decision; the grader writes no sentinel and promotes nothing.
...
Step 0 — preconditions, all mechanical, stop with ok=false on any failure:
  a. python3 -c "from research.swarm.lib import registrar as r; print(r.verify('${FROZEN}'))" must print True (else status FROZEN_SHA_MISMATCH).
  b. ${RUN_DIR}/committee/economics.json AND ${RUN_DIR}/committee/statistics.json must each contain a vote for id "${ID}" with pass true (else status NOT_A_COMMITTEE_PASS — this stage runs only on a committee pass).
  c. ${OUT_HOLDOUT} must NOT exist (else status HOLDOUT_ALREADY_READ — the one read was already spent; do not read again, do not delete it).
  d. git: record base_branch = output of "git rev-parse --abbrev-ref HEAD" and base_sha = output of "git rev-parse HEAD". Then create the build branch FROM THIS HEAD: "git checkout -b ${BRANCH}". If that branch already exists, "git checkout ${BRANCH}" and set branch_existed true (report; do not delete or reset it). If the working tree has unstaged changes, leave them exactly as they are.
...
${VERDICT_LINE}
  ...
  ## Holdout (read once) — write exactly this, nothing more: "PENDING — the single registered holdout read happens AFTER this document is committed, by the command: ${HOLDOUT_CMD} (writes ${OUT_HOLDOUT} and ${TRADES_HOLDOUT}; the train out.json is never touched). Decision rule fixed now: ci95 upper bound < 0 → DEAD-AT-HOLDOUT (no build); n < ${HOLDOUT_MIN_N} or ci95 null → HOLDOUT-INSUFFICIENT (no build; owner decision); otherwise → BUILD. The holdout numbers are recorded below and never used to change anything above."
...
Return {ok, status ("REGISTERED" on success, else one of FROZEN_SHA_MISMATCH / NOT_A_COMMITTEE_PASS / HOLDOUT_ALREADY_READ / ERROR), base_branch, base_sha, branch_existed, prereg_path, commit_sha, error}.`
...
1. TESTS FIRST — ${TEST}, mirroring tests/test_donchian_slot.py in structure (sandbox fixture, pure-module golden cases, AST wiring test, bare bot + FakeExchange). Cover:
   (a) signal parity: load signals() from ${SIGNAL_PY} (importlib from the path; read-only) and the slot module's signal function; assert exact equality of the two series on a synthetic OHLCV frame AND on real TRAIN data of one universe symbol (research.swarm.lib.load_data.load_ohlcv(sym, tf, era="train", dataset=<spec.dataset>)) — this is the transcription check against the frozen file;
   ...
   (f) bare-bot orchestration (object.__new__(botmod.Phmex2Bot) + FakeExchange, no network): a closed-bar signal opens ONE paper position at the fresh price with notional = the module's NOTIONAL_USDT and margin == notional; the same closed bar is not evaluated twice (bar stamp); a later bar touching SL closes it with reason stop_loss at the SL level; slot.enabled False (killed) → no entry and the evaluator returns before fetching; _slot_entries_blocked() True → no NEW entry, exits still run; paper_mode False → NO exchange call of any kind (FakeExchange.calls must stay empty) and one error log per UTC day;
   (g) adjudicator (in tests/test_lab_adjudicator.py, same style as the sr_bounce_v2 tests): grade_${ID} returns WATCH below n=${VERDICT_N}; KILL at n>=${VERDICT_N} with net<=0; KILL at net<=${KILL_NET_USD.toFixed(2)} with n<${VERDICT_N}; PASS at n>=${VERDICT_N} with a CI95 lower bound > 0 (use a clearly positive synthetic ledger); INCONCLUSIVE at n>=${VERDICT_N}, net>0, CI lower<=0; rows with closed_at < registered_ts are ignored; the grader writes no file.
...
4. Live-order path: any code path for this slot that can call an exchange method other than get_ohlcv/get_ticker, or that acts when not slot.paper_mode without returning → BLOCK. paper_mode=True, loss_cap_usdt=-999.0, kelly_min_trades=10**9, durable_trail_enabled=False in the StrategySlot call; strategy_name not in strategies.STRATEGIES.
5. Kill honoured every cycle: the evaluator checks slot.enabled before fetching data; killed/disabled → no entry; ${KILL_FILE} needs no code beyond that (the generic .kill_* loop already exists) → else BLOCK.
6. Adjudicator vs prereg: EXPERIMENTS["${ID}"] verdict_n == ${VERDICT_N}, kill_net_usd == ${KILL_NET_USD.toFixed(1)}, inconclusive_hard_n == ${INCONCLUSIVE_HARD_N}, registered_ts == the epoch of ${NOW}; grade_${ID} implements KILL / PASS / INCONCLUSIVE / WATCH exactly as the "## Verdict line (frozen)" section of ${PREREG} states, sums net_pnl as-is, uses bootstrap_ci.mean_ci (no hand-rolled resampler), writes nothing; the digest line is appended without renumbering existing results — else BLOCK.
...
8. Scope: the first two commits after ${baseSha} are the prereg commits and may touch only ${PREREG}, ${OUT_HOLDOUT}, ${TRADES_HOLDOUT}; every later commit (git show --stat <sha>) touches only files in {${ALLOWED_FILES.join(', ')}}; nothing under research/swarm/runs/, research/swarm/kb/, research/swarm/lib/ or docs/ is modified by the implement/fix commits; no .env, trading_state*.json, sentinel, launchd, logs/ or data files anywhere in ${baseSha}..HEAD; no launchctl/kill/pkill/main.py invocation anywhere in the diff → else BLOCK.
...
const fixPrompt = (reasons) => `${RULES}
FIX seat (one round only). The reviewer BLOCKED branch ${BRANCH} for these reasons:
${reasons.map((r, i) => `${i + 1}. ${r}`).join('\n')}
Fix ONLY what the reasons name, inside ${ALLOWED_FILES.join(', ')}; never the frozen spec, signal.py, research/swarm/lib/, or ${PREREG}. If a reason can only be satisfied by changing a frozen parameter or the signal, do NOT do it — return ok=false with error "requires a frozen change: <which>" (that is a new thesis, not a fix). Keep the tests honest: fix code to pass tests, never weaken a test to pass code. Run python3 -m pytest -q (baseline "${SUITE_BASELINE}" + new tests). Commit with explicit paths: git commit -m "fix(${ID}): review round — <what>" -m "${TRAILER}". Return {ok, files_changed, commit_sha, pytest_tail, notes, error}.`

const reconcilePrompt = (result, holdout, review) => `${RULES}
RECONCILER (build stage; append-only kb writes; commit on ${BRANCH}). Build result: ${result}. Holdout read: ${JSON.stringify(holdout)}. Review: ${JSON.stringify(review ? { verdict: review.verdict, reasons: review.reasons } : null)}.
- If result == "BUILT": append ONE row to ${KB}/SURVIVORS.md in that file's existing column format (run = ${RUN_ID}, id = ${ID}, train n / net bps / CI95 / WR / p* from ${OUT_TRAIN}, prereg doc = ${PREREG}, status = "built on ${BRANCH} — holdout n=<n> net_bps=<x> ci95=<[lo, hi]> (${OUT_HOLDOUT}); awaiting Gate B: owner go + /pre-restart-audit").
- If result == "DEAD_AT_HOLDOUT": append ONE row to ${KB}/DEAD_LIST.md, EXACTLY the format | n | family | why | source | date | (five cells, one line, no pipe characters inside a cell) with n = max existing n + 1 (python3 -c "from research.swarm.lib import kb_check as k; from pathlib import Path; print(max(r[0] for r in k.dead_rows(Path('${KB}/DEAD_LIST.md'))))"), family = ${ID}, why = "dead at the registered holdout read: n=<n>, net_bps_mean=<x>, ci95=<[lo, hi]> upper < 0 (${OUT_HOLDOUT}); train prior n=<n> net_bps=<x> (${OUT_TRAIN})", source = ${PREREG}, date = ${TODAY}.
- Otherwise (HOLDOUT_INSUFFICIENT, HOLDOUT_ERROR, REVIEW_BLOCKED, IMPLEMENT_FAILED, PREREG_*): no SURVIVORS/DEAD_LIST row; append ONE dated line to ${KB}/LESSONS.md: "- ${TODAY} — build ${ID} (run ${RUN_ID}) stopped at ${result}: <one-line cause with the file path>. Rule: <one process rule>".
- In every case where the holdout was read, ALSO append one LESSONS line recording it: "- ${TODAY} — ${ID}: holdout read once at ${NOW} → ${OUT_HOLDOUT} (decision ${result}); the branch ${BRANCH} holds the only record — never delete it unmerged, never re-read holdout for this thesis".
Never edit or delete existing rows or lines; never touch CONSTRAINTS.md, STANDARDS.md, DATA.md or owner_trades/. Then run exactly: python3 -m research.swarm.lib.kb_check — if it does not print KB OK, fix ONLY the row(s)/line(s) you just wrote and re-run until it does. Commit: git add ${KB}/SURVIVORS.md ${KB}/DEAD_LIST.md ${KB}/LESSONS.md (only those you changed) && git commit -m "kb(${ID}): build-stage reconcile — ${result}" -m "${TRAILER}". Return the kb_check output verbatim plus every row/line you added with its file path.`
...
const PREREG_SCHEMA = { type: 'object', properties: {
  ...
  branch_existed: { type: 'boolean' }, prereg_path: { type: 'string' }, commit_sha: { type: 'string' }, error: { type: 'string' } },
  required: ['ok', 'status'] }
...
function holdoutDecision(h) {
  ...
  if (h.n == null || h.n < HOLDOUT_MIN_N || !Array.isArray(h.ci95) || h.ci95.length !== 2) return 'HOLDOUT_INSUFFICIENT'
  if (h.ci95[1] < 0) return 'DEAD_AT_HOLDOUT'
  return 'BUILD'
}

const stop = (result, extra) => ({ run_id: RUN_ID, thesis_id: ID, result, branch: BRANCH, prereg_doc: PREREG, ...extra,
  next: 'Stopped. Nothing restarted, no launchd job touched, no order placed, nothing merged. Owner decides what happens to the branch.' })
...
if (!prereg?.ok) {
  ...
  log(`prereg stopped: ${status} — ${prereg?.error ?? 'seat returned null'}`)
  phase('Reconcile')
  const reconcile = await agent(reconcilePrompt(`PREREG_${status}`, null, null), { label: 'reconcile', phase: 'Reconcile', effort: 'medium' })
  return stop(`PREREG_${status}`, { prereg, reconcile })
}
const BASE_SHA = prereg.base_sha
log(`prereg committed ${prereg.commit_sha ?? '?'} on ${BRANCH} (base ${prereg.base_branch ?? '?'}@${BASE_SHA ?? '?'}${prereg.branch_existed ? ', branch pre-existed' : ''}); holdout read follows — the decision rule is already in the doc`)
...
if (decision !== 'BUILD') {
  phase('Reconcile')
  const reconcile = await agent(reconcilePrompt(decision, holdout, null), { label: 'reconcile', phase: 'Reconcile', effort: 'medium' })
  return stop(decision, { base_sha: BASE_SHA, holdout, prereg_doc_text: preregDoc, reconcile })
}
...
if (!impl?.ok) {
  ...
  phase('Reconcile')
  const reconcile = await agent(reconcilePrompt('IMPLEMENT_FAILED', holdout, null), { label: 'reconcile', phase: 'Reconcile', effort: 'medium' })
  return stop('IMPLEMENT_FAILED', { base_sha: BASE_SHA, holdout, impl, reconcile })
}
log(`implemented ${impl.commit_sha ?? '?'}: ${(impl.files_changed ?? []).join(', ') || 'files not reported'}`)

// ---------- Phase 3: Review (judge model; at most one fix round) ----------
...
phase('Review')
let review = await agent(reviewPrompt(BASE_SHA, 1), judge({ label: 'review:1', phase: 'Review', schema: REVIEW_SCHEMA, effort: 'high' }))
let fix = null
if (review?.verdict === 'BLOCK') {
  log(`review round 1 BLOCKED (${review.reasons.length} reasons) — one fix round, then a final review`)
  fix = await agent(fixPrompt(review.reasons), { label: 'fix', phase: 'Review', schema: IMPL_SCHEMA, effort: 'high' })
  review = fix?.ok
    ? await agent(reviewPrompt(BASE_SHA, 2), judge({ label: 'review:2', phase: 'Review', schema: REVIEW_SCHEMA, effort: 'high' }))
    : { verdict: 'BLOCK', reasons: [`fix seat failed: ${fix?.error ?? 'returned null'}`, ...review.reasons], pytest_tail: fix?.pytest_tail ?? '' }
}
if (!review) review = { verdict: 'BLOCK', reasons: ['reviewer returned null — no approval exists'], pytest_tail: '' }
const filesChanged = [...new Set([...(impl.files_changed ?? []), ...(fix?.files_changed ?? [])])]
const result = review.verdict === 'APPROVE' ? 'BUILT' : 'REVIEW_BLOCKED'
log(`review: ${review.verdict}${review.verdict === 'BLOCK' ? ` — ${review.reasons.slice(0, 3).join(' | ')}` : ''}`)

// ---------- Phase 4: Reconcile (kb row on the branch) and STOP before Gate B ----------
phase('Reconcile')
const reconcile = await agent(reconcilePrompt(result, holdout, review), { label: 'reconcile', phase: 'Reconcile', effort: 'medium' })
return {
  ...
  files_changed: filesChanged, commits: [prereg.commit_sha, impl.commit_sha, fix?.commit_sha].filter(Boolean),
  review, reconcile,
  next: result === 'BUILT'
    ? `Gate B (human): owner reviews branch ${BRANCH} (git log ${BASE_SHA}..${BRANCH}), runs /pre-restart-audit, and says "go" before any restart. build.js restarted nothing, touched no launchd job, placed no order, merged nothing; the working tree is checked out on ${BRANCH}.`
    : `Review BLOCKED after one fix round — branch ${BRANCH} stays unmerged for the owner to inspect; nothing restarted, nothing merged.`,
}
