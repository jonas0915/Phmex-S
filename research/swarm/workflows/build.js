...
Step 0 — preconditions, all mechanical, IN THIS ORDER, stop with ok=false (and create nothing, check out nothing) on the first failure:
  ...
  c2. The frozen spec must carry the portfolio cap (STANDARDS #17): python3 -c "import json; print(json.load(open('${FROZEN}'))['thesis']['spec'].get('max_concurrent'))" must print an integer >= 1, not None (else status SPEC_MISSING_MAX_CONCURRENT — the spec was frozen before registrar filled max_concurrent; it is never edited — the thesis is re-registered through the desk, which freezes the cap; stop, create nothing).
  c3. Tradeable universe (STANDARDS #18): python3 -m research.swarm.lib.universe_check --frozen ${FROZEN} must print UNIVERSE OK and exit 0 — every spec.universe symbol must be an active Phemex market (the one network call in Step 0; a delisting after the research cache ends is invisible to the screen — GIGGLE, LESSONS 2026-09-21). Else status UNIVERSE_UNTRADEABLE with error = the CLI output verbatim; the frozen spec is never edited — a pruned universe is a new thesis through desk.js.
...
Step 1 — write ${PREREG} in the house format (tone and section shape of docs/superpowers/specs/2026-09-03-mr-edge-search-prereg.md and docs/superpowers/specs/2026-07-30-sr-bounce-v2-fixed-geometry-prereg.md). Sections, in this order:
  ...
  **Thesis**: mechanism, counterparty, prediction — VERBATIM from the "thesis" object in ${FROZEN} (fields mechanism / counterparty / prediction), plus source_urls (from the same thesis object) and evidence_grade (from ${RUN_DIR}/gate_kept.json, the gatekeeper's kept entry for this id).
  ## Frozen data — dataset, universe, timeframe, tp_bps, sl_bps, max_hold_bars, max_concurrent, expected_trades_per_week, doa_line (all from ${FROZEN}; max_concurrent is the FROZEN value, quoted as frozen — never recomputed here); frozen spec path + its sha256 field; signal sha256 from ${OUT_TRAIN}; train_span from ${OUT_TRAIN}; holdout = final 25% of the dataset range per load_data.holdout_start (state it as the rule, do not compute dates from holdout rows). If the frozen spec's dataset is "long_1h", this section also records the caveat verbatim: "holdout window overlaps mr_edge train; treat the holdout read as a sanity read only" (cross-dataset holdout caveat, STANDARDS #6).
  ## Economics — $200 basis; notional per trade = the printed output of python3 -c "from research.swarm.lib import fee_math as f; print(f.position_notional())"; c = fee_math.C_BPS (print it); p* = p_star from ${OUT_TRAIN}; lot_check per symbol from ${OUT_TRAIN}; PORTFOLIO CAP (STANDARDS #17): max_concurrent = <frozen value from ${FROZEN}>, derivation = the printed output of python3 -c "from research.swarm.lib import fee_math as f; print(f.max_concurrent(<sl_bps from ${FROZEN}>))" (state the formula: floor(${(-KILL_NET_USD).toFixed(2)} / (notional × (sl_bps + c) / 1e4)), floored at 1) — the printed value must equal the frozen value; if it does not, STOP with ok=false, status ERROR, error "max_concurrent derivation mismatch" (the spec is never edited); state in one sentence that the ${KILL_NET_USD.toFixed(2)} USD cap is sized so that ONE simultaneous cluster of max_concurrent stops cannot alone breach it, that the screen admitted trades under this cap (portfolio block of ${OUT_TRAIN}: n_unconstrained / n_admitted / n_dropped, each with the path) and that the slot enforces it live (MAX_CONCURRENT in ${MODULE}); if max_hold_bars × bar size > 8h, state the funding exposure per CONSTRAINTS. Paper convention: 1x paper book, margin recorded AS the notional (Donchian convention, donchian_slot.py / _donchian_open_paper), so USD PnL = notional × price move and ROI% = price move %.
...
${VERDICT_LINE}
  ## Prior (train, read before this registration) — from ${OUT_TRAIN}: n, net_bps_mean, ci95, wr vs p_star, trades_per_week, time_to_verdict_weeks, portfolio.max_concurrent / n_unconstrained / n_admitted / n_dropped (these are the ADMITTED-set stats: n and every statistic already reflect the cap); from ${SCREEN_DIR}/audit.json: verdict, numbers.p_boot; from ${RUN_DIR}/committee/bh.json: this id's BH row; from ${RUN_DIR}/committee/economics.json and statistics.json: the two votes' reasons; from ${SCREEN_DIR}/out.robust_plus.json and out.robust_minus.json: net_bps_mean of each. EVERY number followed by the path it came from. Missing file → "not run".
...
Return {ok, status ("REGISTERED" on success, else one of PREREG_BRANCH_EXISTS / HOLDOUT_ALREADY_READ / FROZEN_SHA_MISMATCH / SPEC_MISSING_MAX_CONCURRENT / UNIVERSE_UNTRADEABLE / NOT_A_COMMITTEE_PASS / ERROR), base_branch, base_sha, prereg_path, commit_sha, error}.`
...
${PARITY_TESTS}
   ...
   (f3) portfolio cap (STANDARDS #17; same bare-bot fixture as (f)): MAX_CONCURRENT in ${MODULE} equals the frozen spec's max_concurrent (read ${FROZEN} in the test); with MAX_CONCURRENT paper positions already open in slot.risk.positions on OTHER symbols, a closed-bar signal on a new symbol opens NOTHING, logs INFO containing "entry skipped — max_concurrent reached", and stamps the bar (st["last_bar_ts"] advances — the skip is final, never retried on a later cycle, exactly as screen.admit_trades drops rather than defers); with MAX_CONCURRENT − 1 open the same signal opens the position; exits on the already-open positions still run while the cap is reached;
...
5b. Portfolio cap (STANDARDS #17): MAX_CONCURRENT in ${MODULE} is a literal equal to spec.max_concurrent in ${FROZEN} (read both); the open path in _evaluate_${ID} is guarded by len(slot.risk.positions) >= MAX_CONCURRENT → INFO log "entry skipped — max_concurrent reached" + bar stamp + no entry, BEFORE _${ID}_open_paper; exits are not gated by it; ${TEST} covers the skip (cap reached → nothing opened, log present, bar stamped) and the open (cap − 1); ${PREREG} "## Frozen data" and "## Economics" state the same value with its fee_math.max_concurrent derivation and the anti-fishing clause lists max_concurrent — any mismatch or missing piece → BLOCK.
