...
signal_py: Python source defining signals(df) -> pd.Series indexed like df with values exactly in {-1, 0, 1}, computed on CLOSED bars only — no .shift(-k), no future index use, no iloc[-1] forming-bar tricks; the screen runs a whole-prefix causality check on every symbol and any lookahead kills the thesis; a signal that returns any other value (NaN is fine, it is filled to 0) also kills it. df has columns open/high/low/close/volume with a UTC DatetimeIndex. Funding is available via research.swarm.lib.load_data.load_reference_funding(symbol) (rate rows {ts, rate}) if the signal needs it — merge_asof on ts and use only rates settled BEFORE the bar. A signal that needs a second symbol (a reference such as BTC) must load it through \`load_data.load_reference(symbol, timeframe, dataset)\` (or \`load_reference_funding\`), never through \`load_ohlcv\`/\`load_funding\` with an era argument — the screen supplies the era, so the same frozen signal is valid in both train and holdout. (STANDARDS #16; a signal_py that calls load_ohlcv/load_funding with an explicit era is rejected at the gate.) Import only pandas/numpy and research.swarm.lib inside signal_py.
...
CROSS-DATASET HOLDOUT CAVEAT (STANDARDS #6): mr_edge train data overlaps the long_1h holdout window (long_1h holdout ≈ 2026-04-23→08-01; mr_edge train ≈ 2026-06-01→08-10; 18 of 19 long_1h symbols also exist in mr_edge) — a thesis on the long_1h dataset may not use mr_edge data dated on or after 2026-04-23 in any exploratory probe or signal; the gatekeeper rejects a long_1h thesis whose probes did.`
...
3. Read signal.py: any .shift(-k) with k>0, negative-index or future-timestamp use, rolling with center=True, iloc[-1]-style forming-bar logic, or use of data outside df/load_reference/load_reference_funding = REFUTED (lookahead); a reference symbol loaded via load_ohlcv/load_funding with an explicit era is a finding (STANDARDS #16). Confirm out.json causality == "PASS".
...
async function closeOut(results, passed, gate, lensSummary) {
  ...
  const critic = await agent(criticPrompt(report), judge({ label: 'critic', phase: 'Close', effort: 'high' }))
  const screenedIds = results.filter(r => r.audit?.screened).map(r => r.id)
