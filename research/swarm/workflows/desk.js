...
const MAX_SCREENS = A.max_screens ?? 5
const DRY = !!A.dry_run
...
4. Check trades.csv: every entry_ts is strictly after the signal bar (entry at next open); no exit_ts after train_span end from out.json; cost applied (net_bps = gross_bps − 11.5 on every row, check with fee_math.C_BPS).
...
const reconcilePrompt = (passed) => `${RULES}
RECONCILER. For EVERY thesis file in ${RUN_DIR}/theses/*.json EXCEPT owner_record_probe.json (an exploratory probe, not a thesis):
- If it was gate-rejected (see ${RUN_DIR}/gate_rejections.json) or its registrar/screen/audit/committee step failed: append ONE row to ${KB}/DEAD_LIST.md. The row format is EXACTLY, verbatim: | n | family | why | source | date | — five cells, one line, no pipe characters inside any cell, where n = (max existing n in the file) + 1 (compute it by reading the file: python3 -c "from research.swarm.lib import kb_check as k; from pathlib import Path; print(max(r[0] for r in k.dead_rows(Path('${KB}/DEAD_LIST.md'))))"), family = the thesis id (${DRY ? 'prefix it "dryrun:" — this run is a dry run; ' : ''}for a gate rejection whose reason starts "not screenable:" write the why as "PARKED (not refuted): <reason>"), why = the decisive reason in one line (cite out.json / audit.json / committee numbers with their paths when they exist, else the dead row it relabeled as "relabel of row <n>", else the source-verification grade), source = ${RUN_DIR}/REPORT.md, date = ${TODAY}.
...
async function closeOut(results, passed, gate, lensSummary) {
  ...
  const reconcile = await agent(reconcilePrompt(passed), { label: 'reconcile', phase: 'Close', effort: 'high' })
