...
Runs signal_expiry_release on every long_1h symbol, simulates with lib.screen.simulate at
...
spec = importlib.util.spec_from_file_location("sig", HERE / "signal_expiry_release.py")
...
trades.to_csv(HERE / f"probe_expiry_release_trades_{tag}.csv", index=False)
(HERE / f"probe_expiry_release_out_{tag}.json").write_text(json.dumps(out, indent=2))
